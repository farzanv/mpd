...
GetAnyIpAddress(struct u_addr *ipaddr, const char *ifname)
{
	...
	if (ipa.s_addr == 0) {
		...
		while (1) {
			...
			/*
			 * if used size is too close to allocated size retry
		...
		for (ifend = (struct ifreq *)(void *)(ifc.ifc_buf + ifc.ifc_len),
			...
			if (ifr->ifr_addr.sa_family == AF_INET) {
				...
				/*
				 * Check that the interface is up; prefer
				 * non-p2p and non-loopback
				 */
...
GetEther(struct u_addr *addr, struct sockaddr_dl *hwaddr)
{
	...
	while (1) {
		...
		/*
		 * if used size is too close to allocated size retry with a
		 * larger buffer
		 */
		if (ifc.ifc_len + 128 < buffsize)
			break;
	...
	/*
	 * Now scan again looking for a link-level address for this
	 * interface
	 */
