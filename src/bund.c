...
BundJoin(Link l)
{
	...
	if (b->n_up == 1) {
		...
		/*
		 * Cancel re-open timer; we've come up somehow (eg, LCP
		 * renegotiation)
		 */
	...
	if (b->n_up == 0) {
		...
		if (b->open && Enabled(&b->conf.options, BUND_CONF_BWMANAGE) &&
			...
			if (b->n_links != 0 || b->conf.linkst[0][0]) {
				/*
				 * wait BUND_REOPEN_DELAY to see if it comes
				 * back up
				 */
...
BundCreate(Context ctx, int ac, char *av[], void *arg)
{
	...
	if (bt) {
		...
		/*
		 * Add bundle to the list of bundles and make it the current
		 * active bundle
		 */
...
BundInst(Bund bt, char *name, int tmpl, int stay)
{
	...
	/*
	 * Add bundle to the list of bundles and make it the current active
	 * bundle
	 */
	for (k = 0; k < gNumBundles && gBundles[k] != NULL; k++);
	if (k == gNumBundles)		/* add a new bundle pointer */
