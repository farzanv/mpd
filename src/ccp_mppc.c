...
MppcDecodeConfigReq(Fsm fp, FsmOption opt, int mode)
{
	...
	case MODE_REQ:
		...
		/*
		 * It doesn't really make sense to encrypt in only one
		 * direction. Also, Win95/98 PPTP can't handle
		 * uni-directional encryption. So if the remote side doesn't
		 * request encryption, try to prompt it. This is broken wrt.
		 * normal PPP negotiation: typical Microsoft.
		 */
		if ((bits & MPPE_BITS) == 0) {
			if (MppcAcceptableMppeType(b, 40))
				bits |= MPPE_40;
