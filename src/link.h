...
enum {
	...
	LINK_CONF_REMOVE_TEE		/* Remove ng_tee from the resulting
					 * chain */
};
...
struct linkst {
	...
	struct ng_ppp_link_stat oldStats;	/* Previous stats for 64bit
						 * emulation */
#endif
...
extern void 
RecordLinkUpDownReason(Bund b, Link l, int up, const char *fmt,
    const char *arg,...);
