	    ...
	    main(int ac, char *av[])
{
	...
	if (ac > MAX_ARGS)
		ac = MAX_ARGS;
	memcpy(args, av, ac * sizeof(*av));	/* Copy to preserve "ps"
						 * output */
	...
	assert(0);
	return (1);			/* Never reached, but needed to
...
FatalSignal(int sig)
{
	...
	/*
	 * If a SIGTERM or SIGINT, gracefully shutdown; otherwise shutdown
	 * now
	 */
