...
IpcpConfigure(Fsm fp)
{
	...
	/*
	 * If any of our links are unable to give receive error indications,
	 * we must tell the peer not to compress the slot-id in VJCOMP
	 * packets (cf. RFC1144). To be on the safe side, we always say
	 * this.
	 */
	...
	/* Request peer's DNS and NBNS servers */
	{
		...
		for (sopt = 0; sopt < 2; sopt++) {
			for (pri = 0; pri < 2; pri++) {
				...
				/*
				 * Add option if we desire it and it hasn't
				 * been rejected
				 */
	...
	doDnsNbns:
			{
				...
				case MODE_REQ:
					...
					FsmAck(fp, opt);	/* he knows what he
								 * wants */
					break;
				case MODE_NAK:	/* we asked for his server,
						 * he's telling us */
					*wantip = hisip;
					break;
				case MODE_REJ:	/* we asked for his server,
						 * he's ignorant */
					IPCP_PEER_REJ(ipcp, opt->type);
					break;
				}
