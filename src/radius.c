...
RadiusStart(AuthData auth, short request_type)
{
	...
	/*
	 * Insert the Message Authenticator RFC 3579 If using EAP this is
	 * mandatory
	 */
...
RadiusPutAuth(AuthData auth)
{
	...
	if (auth->proto == PROTO_CHAP || auth->proto == PROTO_EAP) {
		...
		case CHAP_ALG_MD5:
			/*
			 * RADIUS requires the CHAP Ident in the first byte
			 * of the CHAP-Password
			 */
...
RadiusGetParams(AuthData auth, int eap_proxy)
{
	...
	while ((res = rad_get_attr(auth->radius.handle, &data, &len)) > 0) {
		...
		case RAD_CLASS:
			...
			/*
			 * libradius already checks the
			 * message-authenticator, so simply ignore it
			 */
			...
			case RAD_VENDOR_SPECIFIC:
				...
				case RAD_VENDOR_MICROSOFT:
					...
					case RAD_MICROSOFT_MS_CHAP_ERROR:
						...
						/*
						 * there is a nullbyte on
						 * the first pos, don't know
						 * why
						 */
	...
	if (auth->acct_type == 0) {
		...
		/*
		 * sanity check, this happens when FreeRADIUS has no
		 * msoft-dictionary loaded
		 */
		...
		/*
		 * If no MPPE-Infos are returned by the RADIUS server, then
		 * allow all
		 */
		/*
		 * MSoft IAS sends no Infos if all MPPE-Types are enabled
		 * and if encryption is optional
		 */
		...
		/*
		 * If Framed-IP-Address is present and Framed-Netmask != 32
		 * add route
		 */
