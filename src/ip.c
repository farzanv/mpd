...
IpAddrInRange(struct u_range *range, struct u_addr *addr)
{
	...
	if (u_rangeempty(range))
		return (1);
	//For compatibility we think that empty range includes any ip.
...
sa_family_t 
u_addrfamily(struct u_addr *addr)
{
	return addr->family;
}

sa_family_t 
u_rangefamily(struct u_range *range)
{
	return range->addr.family;
}

char   *
u_addrtoa(struct u_addr *addr, char *dst, size_t size)
{
	dst[0] = 0;
...
char   *
u_rangetoa(struct u_range *range, char *dst, size_t size)
{
...
void 
u_addrcopy(const struct u_addr *src, struct u_addr *dst)
{
	memcpy(dst, src, sizeof(struct u_addr));
}

void 
u_rangecopy(const struct u_range *src, struct u_range *dst)
{
	memcpy(dst, src, sizeof(struct u_range));
}

void 
u_addrclear(struct u_addr *addr)
{
	memset(addr, 0, sizeof(struct u_addr));
}

void 
u_rangeclear(struct u_range *range)
{
	memset(range, 0, sizeof(struct u_range));
}

void 
in_addrtou_addr(const struct in_addr *src, struct u_addr *dst)
{
	u_addrclear(dst);
	dst->family = AF_INET;
	dst->u.ip4 = *src;
...
void 
u_addrtoin_addr(const struct u_addr *src, struct in_addr *dst)
{
	*dst = src->u.ip4;
}

void 
u_addrtoin6_addr(const struct u_addr *src, struct in6_addr *dst)
{
	*dst = src->u.ip6;
}
...
int 
u_addrempty(struct u_addr *addr)
{
	int i;
...
int 
u_rangeempty(struct u_range *range)
{
	return u_addrempty(&range->addr);
}

int 
u_rangehost(struct u_range *range)
{
	switch (range->addr.family) {
	case AF_INET:
...
void 
u_rangetosockaddrs(struct u_range *range, struct sockaddr_storage *dst, struct sockaddr_storage *msk)
{
...
void 
u_addrtosockaddr(struct u_addr *addr, in_port_t port, struct sockaddr_storage *dst)
{
	memset(dst, 0, sizeof(struct sockaddr_storage));
...
void 
sockaddrtou_addr(struct sockaddr_storage *src, struct u_addr *addr, in_port_t *port)
{
...
int 
u_addrcompare(const struct u_addr *addr1, const struct u_addr *addr2)
{
	int i;
...
int 
u_rangecompare(const struct u_range *range1, const struct u_range *range2)
{
