...
LcpConfigure(Fsm fp)
{
	...
	for (i = 0; i < LCP_NUM_AUTH_PROTOS; i++) {
		if (Enabled(&l->conf.options, lcp->want_protos[i]->conf) && lcp->want_auth == 0) {
			...
			/*
			 * avoid re-requesting this proto, if it was nak'd
			 * by the peer
			 */
...
LcpNewPhase(Link l, enum lcp_phase new)
{
	...
	/*
	 * Sanity check transition (The picture on RFC 1661 p. 6 is
	 * incomplete)
	 */
	switch (old) {
	case PHASE_DEAD:
...
LcpDecodeConfig(Fsm fp, FsmOption list, int num, int mode)
{
	...
	for (k = 0; k < num; k++) {
		...
		case TY_AUTHPROTO:	/* authentication protocol */
			{
				...
				case MODE_REQ:
					...
					/*
					 * no other acceptable auth-proto
					 * found
					 */
				...
				case MODE_NAK:
					...
					/*
					 * let us check, whether the
					 * requested auth-proto is enabled
					 */
					...
					/*
					 * Remove the disabled proto from my
					 * list
					 */
		...
		case TY_CALLBACK:	/* Callback */
			...
			switch (mode) {
			case MODE_REQ:	/* we only support peer calling us
					 * back */
				FsmRej(fp, opt);
				break;
