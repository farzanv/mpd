...
static int 
FsmExtractOptions(Fsm fp, u_char *data,
...
FsmRecvConfigAck(Fsm fp, FsmHeader lhp, Mbuf bp)
{
	...
	/*
	 * XXX We should verify the contents are equal to our last sent
	 * config-req
	 */
...
FsmRecvConfigRej(Fsm fp, FsmHeader lhp, Mbuf bp)
{
	...
	/*
	 * XXX should verify contents are a subset of previously sent
	 * config-req
	 */
...
FsmRecvCodeRej(Fsm fp, FsmHeader lhp, Mbuf bp)
{
	...
	if (fp->type->RecvCodeRej)
		...
		case CODE_IDENT:
		case CODE_TIMEREM:
		default:		/* if we don't know it, that makes two
					 * of us */
			fatal = FALSE;
			break;
		}
...
FsmSendIdent(Fsm fp, const char *ident)
{
	...
	/*
	 * Leave magic zero unless fully opened, as IDENT can be sent
	 * anytime
	 */
	if (fp->state != ST_OPENED)
		self_magic = 0;
...
FsmEchoTimeout(void *arg)
{
	...
	default:
		...
		/* fall through */
	case 1:			/* one interval of silence elapsed;
					 * send first echo request */
