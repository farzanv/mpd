...
#ifdef USE_IPFW
struct acl_pool *rule_pool = NULL;	/* Pointer to the first element in the
					 * list of rules */
struct acl_pool *pipe_pool = NULL;	/* Pointer to the first element in the
					 * list of pipes */
struct acl_pool *queue_pool = NULL;	/* Pointer to the first element in the
					 * list of queues */
struct acl_pool *table_pool = NULL;	/* Pointer to the first element in the
					 * list of tables */
int	rule_pool_start = 10000;	/* Initial number of ipfw rules pool */
int	pipe_pool_start = 10000;	/* Initial number of ipfw dummynet
					 * pipe pool */
int	queue_pool_start = 10000;	/* Initial number of ipfw dummynet
					 * queue pool */
...
static const struct bpf_insn gTCPSYNProg[] = {
	 ...
	 /*02*/ BPF_STMT(BPF_LD + BPF_H + BPF_ABS, 6),	/* A <- fragmentation
							 * offset */
	...
	if (ready) {
		...
		while (acls != NULL) {
			if (acls->real_number == 0) {
				if (acls->number == prev_number) {	/* ACL list is presorted
									 * so we need not
									 * allocate if equal */
...
IfaceIdleTimeout(void *arg)
{
	...
	if (iface->idleStats.recvFrames + iface->idleStats.xmitFrames <
		...
		iface->traffic[0] = TRUE;
	} else {			/* no demand traffic for a whole idle
					 * timeout period? */
...
IfaceChangeAddr(Bund b, int add, struct u_range *self, struct u_addr *peer)
{
	...
	case AF_INET6:
		...
		if (res == -1) {
			if (add && errno == EEXIST) {
				/*
				 * this can happen if the kernel has already
				 * automatically added the same link-local
				 * address - ignore the error
				 */
				res = 0;
			} else {
...
#ifdef USE_NG_BPF
	IfaceShutdownLimits(b);		/* Limits must shutdown first to save
					 * final stats. */
#endif
#ifdef USE_NG_NAT
	if (b->iface.nat_up)
...
IfaceInitNAT(Bund b, char *path, char *hook)
{
	...
	if (u_addrempty(&nat->alias_addr)) {
		ip.s_addr = 1;
		//Set something just to make it ready
	} else {
...
#endif					/* defined(USE_NG_TCPMSS) ||
					 * (!defined(USE_NG_TCPMSS) &&
					 * defined(USE_NG_BPF)) */
...
#define	ACL_MAX_PARAMS	7		/* one more then max number of
					 * arguments */
				int ac;
...
#define ACL_BUF_SIZE	256*1024
				...
				if (l->next && strcasecmp(av[0], "all")) {
					/*
					 * If there is next limit and there
					 * is possible nomatch, then pass
					 * nomatch there.
					 */
				...
				/*
				 * Remember how to collect stats for this
				 * limit
				 */
				if (l->name[0]) {
...
IfaceSetDescr(Bund b, const char *template)
{
	...
	struct ifreq ifr;
	int s;
	unsigned int ifdescr_maxlen = 1024;	/* used as limit for old
						 * kernels */
	char *newdescr;
	...
	if ((src = template) != NULL) {
		...
		/*
		 * Will use Mstrdup() later for iface->ifdescr to free extra
		 * memory
		 */
		...
		while (proceed && *src && newdescr < limit) {
			if (*src != '%') {	/* ordinary symbol, just copy
						 * it and proceed */
				*newdescr++ = *src++;
				continue;
			}
			if (!*(src + 1)) {	/* '%' at the end of template,
						 * just copy */
				*newdescr++ = *src++;
				continue;
			}
	...
	proceed = 0;				\
      ...
      *newdescr++ = '-';			\
			...
			case 'U':
				...
				break;
#undef DST_COPY
			default:	/* unrecognized specification, just
					 * copy */
				*newdescr++ = '%';
				if (newdescr < limit)
					*newdescr++ = *src;
	...
	if (mib[0] == 0)
		return (0);		/* do not bother kernel if it is too
					 * old */

	if (ifname == NULL || *ifname == '\0')
		return (0);		/* we have not set system interface
					 * name yet */
	...
	if (!newdescr || !*newdescr) {	/* empty description or clearing
					 * request */
