...
static int 
ChapHash(Link l, int alg, u_char *hash_value, u_char id,
...
ChapSendChallenge(Link l)
{
	...
	/*
	 * Put random challenge data in buffer (only once for Microsoft
	 * CHAP)
	 */
	...
	case CHAP_CHALLENGE:
		{
			...
			/*
			 * Don't respond to a challenge that looks like it
			 * came from us and has the wrong origination value
			 * embedded in it. This avoids a security hole
			 * associated with using the same CHAP password to
			 * authenticate in both directions on a link.
			 */
			idFail = 0;
			do {
				...
				/*
				 * Copy challenge bits and extract
				 * origination value
				 */
				...
				/*
				 * ID bytes match; origination value must be
				 * opposite. Note this assumes that if we
				 * can tell the origination direction of a
				 * link, then so can the peer.
				 */
				switch (l->originate) {
			...
			/*
			 * Need to remember MS-CHAP stuff for use with MPPE
			 * encryption
			 */
	...
	case CHAP_ALG_MSOFT:
		{
			...
			/*
			 * We don't generate the LANManager hash because
			 * it's too insecure
			 */
	...
	case CHAP_ALG_MSOFTv2:
		{
			...
			if (local) {	/* generate reverse 'peer challenge' */
				...
				val->flags = 0;	/* rfc2759, paragraph 4 says
						 * that flags MUST be zero */
			}
