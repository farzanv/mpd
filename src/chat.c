...
static void 
ChatAddTimer(ChatInfo c, const char *set,
    u_int secs, const char *label);
static void 
ChatAddMatch(ChatInfo c, int exact,
...
static int 
ChatVarSet(ChatInfo c, const char *name,
...
static int 
ChatVarExtract(const char *string,
...
static void 
ChatDumpBuf(ChatInfo c, const char *buf,
		...
		for (match = c->matches; match; match = match->next) {
			if (match->exact) {
				...
				if (pmatch[0].rm_eo > 0 && c->lineBuf[pmatch[0].rm_eo - 1] == '\n') {
					pmatch[0].rm_eo--;
					flags &= ~REG_NOTEOL;	/* this is a complete
								 * line */
	...
	case CMD_WAIT:
		if (ac == 2) {
			if (!ChatDecodeTime(c, av[1], &secs))
				...
				ChatCancel(c, DEFAULT_SET);	/* Simulate immediate
								 * expiration */
		} else {
...
/* Find variable if it exists, otherwise create and add to list */
	...
	if ((var = ChatVarFind(head, name))) {
		...
		/*
		 * Replace value; free it after expanding (might be used in
		 * expansion)
		 */
...
ChatMatchChar(struct cm_exact *ex, char ch)
{
	...
	/*
	 * Account for zero length pattern string -- match on next input
	 * char
	 */
	if (len == 0)
		return 1;
