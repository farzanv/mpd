...
enum {
 ...
 /* Binary options */
enum {
	PPTP_CONF_OUTCALL,		/* when originating, calls are
					 * "outgoing" */
	PPTP_CONF_DELAYED_ACK,		/* enable delayed receive ack
					 * algorithm */
	PPTP_CONF_ALWAYS_ACK,		/* include ack with all outgoing data
					 * packets */
	PPTP_CONF_RESOLVE_ONCE,		/* Only once resolve peer_addr */
#if NGM_PPTPGRE_COOKIE >= 1082548365
	PPTP_CONF_WINDOWING		/* control (stupid) windowing
					 * algorithm */
#endif
};
...
PptpOpen(Link l)
{
	...
	case PHYS_STATE_CONNECTING:
		if (pptp->originate)	/* our call to peer is already in
					 * progress */
			break;
		if (pptp->outcall) {
			...
			if (PptpHookUp(l) < 0) {
				PptpDoClose(l);
				/*
				 * We should not set state=DOWN as
				 * PptpResult() will be called once more
				 */
				break;
			}
		...
		return;			/* wait for peer's incoming pptp call
					 * to complete */
...
PptpHookUp(Link l)
{
	...
	if (tun == NULL) {
		...
		/*
		 * increase recvspace to avoid packet loss due to very small
		 * GRE recv buffer.
		 */
    ...
    const char *subAddress)
{
	...
	/*
	 * Find a suitable link; prefer the link best matching peer's IP
	 * address
	 */
