...
static void 
ModemChatConnectResult(void *arg,
    int rslt, const char *msg);
static void 
ModemChatIdleResult(void *arg, int rslt,
    const char *msg);
	...
	if (m->fd >= 0) {		/* Device is already open.. */
		if (m->answering) {	/* We just answered a call */
			...
			Log(LG_PHYS2, ("[%s] MODEM: Stop idle script then dial back",
			    l->name));
			ModemDoClose(l, TRUE);	/* Stop idle script then dial
						 * back */
		}
	} else
...
failed:
	...
	/*
	 * Set modem's reported connection speed (if any) as the link
	 * bandwidth
	 */
...
ModemChatIdleResult(void *arg, int result, const char *msg)
{
	...
	/*
	 * See what script wants us to do now by checking variable
	 * $IdleResult
	 */
...
ModemInstallNodes(Link l)
{
	...
	if (ioctl(m->fd, TIOCSETD, &ldisc) < 0) {
		...
		/*
		 * Installation of the tty node type should be automatic,
		 * but isn't yet. The 'mkpeer' below will fail, because you
		 * can only create a ng_tty node via TIOCSETD; however, this
		 * will force a load of the node type.
		 */
		if (errno == ENODEV) {
...
ModemStat(Context ctx)
{
	...
	if (ctx->lnk->state != PHYS_STATE_DOWN) {
		...
		/*
		 * Set modem's reported connection speed (if any) as the
		 * link bandwidth
		 */
