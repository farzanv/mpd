...
static void 
PptpStartCtrlConnRequest(PptpCtrl c,
    struct pptpStartCtrlConnRequest *m);
static void 
PptpStartCtrlConnReply(PptpCtrl c,
    struct pptpStartCtrlConnReply *m);
static void 
PptpStopCtrlConnRequest(PptpCtrl c,
    struct pptpStopCtrlConnRequest *m);
static void 
PptpStopCtrlConnReply(PptpCtrl c,
...
static void 
PptpCallClearRequest(PptpChan ch,
...
static void 
PptpCtrlDialResult(void *cookie,
...
static void 
PptpCtrlOrigCall(int incoming, struct pptpctrlinfo *cinfo,
...
static PptpChan 
PptpCtrlGetChan(PptpCtrl c, int chanState, int orig,
...
static PptpChan 
PptpCtrlFindChan(PptpCtrl c, int type,
...
PptpCtrlListen(struct u_addr *ip, in_port_t port)
{
	...
	/*
	 * See if we're already have a listener matching this address and
	 * port
	 */
...
PptpCtrlMsg(PptpCtrl c, int type, void *msg)
{
	...
	/*
	 * See if this message qualifies as the reply to a previously sent
	 * message
	 */
    ...
    struct u_addr *peer_addr, in_port_t peer_port, char *buf, size_t bsiz)
{
	...
	if (orig) {
		/*
		 * See if we're already have a control block matching this
		 * address and port
		 */
...
PptpCtrlDialResult(void *cookie, int result, int error, int cause, int speed)
{
	...
	rep.cause = cause;
	rep.speed = speed;
	rep.ppd = PPTP_PPD;		/* XXX should get this value from link
					 * layer */
